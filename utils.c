...
void	print_instruc(t_push_swap *ps, char *instruc)
{
	int		max;
...
void	print_separation(t_push_swap *ps, int max)
{
	max = ps->max_len;
	while (max--)
		ft_putstr_fd("\033[1;36m-", 1);
...
void	print_stack_b(t_push_swap *ps, t_stack **stack_b, int len, int max)
{
	while (max--)
		ft_putstr_fd(" ", 1);
	ft_putstr_fd(" | ", 1);
